 ...
 * 		all USERNAME    -  finds and prints all the login and logout sessions of specified USERNAME
 * 		total USERNAME  -  prints the total duration of each session
 * 		quit            -  exits program
 * 
...
public class LoginStats {
		 ...
		 * The first string of the args array is the name of 
		 * the input file containing the data for the login/ logouts
		 */
	public static void main(String[] args) {
		 ...
		 * 
		 * File opening, error handling, and receiving input from
		 * command line: From lines 38-60 in ColorConverter.java
		...
		while (!userValue.equalsIgnoreCase("quit")) {
			...
			if (!userValue.equalsIgnoreCase("quit")) {
				...
				commandInput = parseUserValue.next();
				userValueUsername = parseUserValue.next();
				Session s = null;
				SortedLinkedList<Session> sList = null;
				long totalDuration = 0;
			
				if (commandInput.equalsIgnoreCase("first")) {
					s = list.getFirstSession(userValueUsername);
					System.out.print(s.toString());
				} else if (commandInput.equalsIgnoreCase("last")){ 
					s = list.getLastSession(userValueUsername);
					System.out.print(s.toString());
				} else if (commandInput.equalsIgnoreCase("all")) {
					sList = list.getAllSessions(userValueUsername);
					// Print every session for specified user
					for (Session sesh: sList) {
						System.out.print(sesh.toString());
					}
				} else if (commandInput.equalsIgnoreCase("total")) {
					totalDuration = list.getTotalTime(userValueUsername);
					System.out.print("\n" + userValueUsername + ", total " 
					+ Helper.durationToString(totalDuration, 2) + "\n");
				}
			}
		}
			userInput.close();	
